...
class TrinoSqlDialectTest {
    private TrinoSqlDialect dialect;
    @Mock
    ...
    void testCreateRemoteMetadataReader() {
        assertThat(this.dialect.createRemoteMetadataReader(), instanceOf(TrinoMetadataReader.class));
    }
    ...
    void testCreateRemoteMetadataReaderConnectionFails(@Mock final Connection connectionMock) throws SQLException {
        ...
        assertThat(exception.getMessage(), containsString("E-VSTR-3"));
    }
    ...
    void testGetCapabilities() {
        ...
        assertAll(
                () -> assertThat(capabilities.getMainCapabilities(), containsInAnyOrder(SELECTLIST_PROJECTION,
                        SELECTLIST_EXPRESSIONS, FILTER_EXPRESSIONS, AGGREGATE_SINGLE_GROUP,
                ...
                () -> assertThat(capabilities.getPredicateCapabilities(),
                        containsInAnyOrder(AND, OR, NOT, EQUAL, NOTEQUAL, LESS, LESSEQUAL, LIKE, LIKE_ESCAPE,
                                BETWEEN, REGEXP_LIKE, IN_CONSTLIST, IS_NULL, IS_NOT_NULL)),
    ...
    @CsvSource({"ABC, \"abc\"", "AbCde, \"abcde\"", "\"tableName, \"\"\"tablename\""})
    @ParameterizedTest
    ...
    @Test
    void testTrinoIdentifierMappingConsistency() throws PropertyValidationException {
        final SqlDialect sqlDialect = testee(Map.of(CONNECTION_NAME_PROPERTY, "MY_CONN", "TRINO_IDENTIFIER_MAPPING",
                "CONVERT_TO_UPPER"));
        sqlDialect.validateProperties();
    }

    @Test
    void testTrinoIdentifierMappingInvalidPropertyValueThrowsException() {
        final SqlDialect sqlDialect = testee(Map.of(CONNECTION_NAME_PROPERTY, "MY_CONN", "TRINO_IDENTIFIER_MAPPING",
                "CONVERT"));
        final PropertyValidationException exception = assertThrows(PropertyValidationException.class,
                sqlDialect::validateProperties);
        assertThat(exception.getMessage(), containsString("E-VSTR-4"));
    }

    @Test
    void testIgnoreErrorsConsistency() {
        final SqlDialect sqlDialect = testee(Map.of(CONNECTION_NAME_PROPERTY, "MY_CONN", IGNORE_ERRORS_PROPERTY,
                "ORACLE_ERROR"));
        final PropertyValidationException exception = assertThrows(PropertyValidationException.class,
                sqlDialect::validateProperties);
        assertThat(exception.getMessage(), containsString("E-VSTR-5"));
    }

    @Test
    void testValidateCatalogProperty() throws PropertyValidationException {
        testee(Map.of(CONNECTION_NAME_PROPERTY, "MY_CONN", CATALOG_NAME_PROPERTY, "MY_CATALOG")).validateProperties();
    }

    @Test
    void testValidateSchemaProperty() throws PropertyValidationException {
        testee(Map.of(CONNECTION_NAME_PROPERTY, "MY_CONN", SCHEMA_NAME_PROPERTY, "MY_SCHEMA")).validateProperties();
    }

    private TrinoSqlDialect testee(final Map<String, String> properties) {
        return testee(new AdapterProperties(properties));
    }

    private TrinoSqlDialect testee(final AdapterProperties properties) {
        return new TrinoSqlDialect(this.connectionFactoryMock, properties, this.exaMetadataMock);
    }
}
